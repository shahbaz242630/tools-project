...
**Phase 0 — Foundations and guardrails. CLOSED, 11 August 2026.** The exit gate is met in full and every clause was _fired_ rather than wired: a change travels branch → CI → staging with no human near a secret (routine now — merging a PR deploys), rollback was run in both directions and verified by the image tags the containers were actually running, and logs were retrieved from the live staging API. The one narrower claim, left standing rather than rounded up: the workflow's **automatic** log step is `if: failure()` and no deploy has ever failed, so that path is wired and unfired.

**Phase 1 — Identity and basic profiles. Built and merged; the gate is one demonstration short.** Its gate is _"automated tests prove users cannot read or modify another user's private data; full signup and profile flow works in staging."_ The first clause is met by test. Sign-up **was** walked on staging on 9 August — just-in-time provisioning wrote the mirror row 45 ms after the first authenticated call, with an `account.provisioned` audit entry — so what is left is the **profile** half of that sentence, on staging, which is now reachable over an SSH tunnel. That is minutes of work rather than a slice, and it is the cheapest phase closure available.

**Phase 2 — Configurable categories and listing MVP. Open, and its exit gate is already met** (since 2.4b: a new category is added by configuration and a listing renders its category-specific fields with no frontend change, demonstrated in a browser). Delivered: 2.1–2.3 configuration and the seller tax profile, 2.4a–c the listing draft with attribute values and transport, 2.5a–b the collection address split and geocoding with a persisted fuzz offset, 2.7a–b fee policy and the inclusive rate card, 2.8a–c publication, pause, resume and moderation state. **Remaining: 2.6** (media, blocked on the domain for the R2 bucket), **2.8c-ii** (telling the owner their listing was hidden — the most urgent, because today nothing does), **2.8d** (the value-migrating re-pin), and **2.9–2.12** (owner dashboard, public listing page, concierge creation, SEO).

**Phases 3–12 have not been started.** No search, bookings, payments, messaging, handover protocol, reviews, disputes or reporting. What exists is a platform that knows who somebody is, protects what it knows about them, can be administered safely, and lets an owner describe and publish an item for rent that the platform can independently hide — **it is not yet a marketplace**: nothing is photographed, searchable or bookable, and there is no public page a listing can be seen on. **Counting phases makes this look like 20% done and that overstates it** — 0 and 1 are scaffolding, and the booking state machine, the ledger and the condition-evidence protocol are each larger than everything built so far.

**The per-slice table lives in the phase handoffs**, not here — one folder per phase under `docs/`, and `phase-02-categories-and-listings/HANDOFF.md` is the one being worked. In summary: Phase 0 delivered the monorepo, primitives, local stack, CI, environment validation, logging, ADRs, the invariant checker, all three applications and the deployment foundation. Phase 1 delivered identity on Clerk with a mirrored `users` table, profiles with a public/private split, an append-only audit log, deletion with real erasure, data export, email correction, and the whole administrative surface — role with MFA at the guard, read-only account and activity views, dual approval on role changes, suspension, and sign-in history. Phase 2 delivered the versioned category with its attribute schema, fee policy and transport options, the listing draft that renders those fields without knowing them, the collection address split across a public and a private table, geocoding behind a port with a persisted fuzz offset, the inclusive total-price rule, and the publication lifecycle — publish, pause, resume, and the platform's own moderation state beside the owner's intent.
