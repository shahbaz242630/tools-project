...
- **`buildPostgresUrl` cannot express `sslmode`, so the API cannot start against Neon.** `psql` connects (libpq defaults to `prefer`); node-postgres refuses outright. When fixed, the deployed value must be **`verify-full`**, not `require` — pg 9 changes `require` to mean encrypted-but-unverified, so writing it that way lets a routine dependency bump downgrade database TLS with nothing failing.
- **Rollback and log retrieval are demonstrated mechanically, not against staging.** The `Deploy rehearsal` CI job proves the mechanism on every PR, and 23 migrations have now been applied to Neon from the published image running on the box — but **by hand, not from CI**, which is precisely the clause the exit gate cares about.
...
**A box now exists** (Hetzner CX23, Falkenstein — ADR 0037) with Docker on it, and the migrations image has run against Neon from it. **The rest of `infra/compose` has still never run for real** — the app stack, the Caddy ingress and the observability stack are rehearsed in CI against the same files and nothing more. The ingress in particular is expected to change: Cloudflare's Tunnel makes an inbound-listening reverse proxy the wrong shape. See `infra/compose/README.md`, and expect to correct it.
