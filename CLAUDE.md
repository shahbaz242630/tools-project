...
**Phase 2 — Configurable categories and listing MVP. Open, and its exit gate has been met since 2.4b** (a new category is added by configuration and a listing renders its category-specific fields with no frontend change, demonstrated in a browser). All 23 slices are delivered. **Remaining: 2.6** (media, blocked on the domain for the R2 bucket) and **2.12** (SEO, unblocked but premature — there is nothing on the internet and one published listing; it got more pressing when `/browse` became indexable, because a search URL per postcode is exactly the canonical and duplicate-content question that slice owns).
