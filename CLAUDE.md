...
- **Search filters compose, and an absent one is absent** (ADR 0046, §8.4): the category predicate lives _inside_ the radius query, because filtering after it is the filter-after-paginate bug; an unchosen filter contributes **no SQL at all** rather than an always-true clause, which is what keeps the unfiltered statement identical to the one the Phase 3 gate was measured against. A search's **metric labels are closed unions and a filter is recorded as a boolean, never as the category** — a category is configuration, so labelling by it grows the series count through an admin form. **URL builders take the whole search**, so a new filter is a compile error in each rather than five call sites to remember.
