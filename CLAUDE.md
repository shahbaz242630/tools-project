...
**Phase 4 — Availability and booking request. OPEN, 16 August 2026.** Seven of ten slices (4.5 was split into 4.5a and 4.5b): **4.1** §7's state machine as pure logic (23 states, 47 transitions, every one of the 482 forbidden pairs refused); **4.2** the `bookings` table and §8.5.1's real `EXCLUDE` constraint, with the simultaneous-acceptance test; **4.3a** the availability data and query; **4.3b** the owner's calendar page, which an owner can now open and use; **4.4a** §8.5.3's maximum rental duration as versioned configuration; **4.4b** the quote engine and the `Quote` record, which also enforces §3.4.2's minimum booking total for the first time; **4.5a** request submission — the terms a booking keeps, §6.2's immutable event history, and §8.6's request deadline as versioned configuration. **Two thirds of the exit gate is met** — §7.1's auto-decline is 4.6. **Something is bookable and nothing is accepted**, and no slice in this phase has a page yet: 4.5b is the renter's panel. **Two findings bind the rest of the phase**: `ACCEPTED` has no one-step release of the calendar, so acceptance must move straight through it in one transaction; and an availability block does not stop a booking at the database level, so the request and acceptance paths must consult `reasonUnavailable` themselves. **And one worth knowing anywhere**: two inserts racing on an exclusion constraint make Postgres report `40P01 deadlock detected` rather than a constraint violation about one race in three — retry, do not translate.
