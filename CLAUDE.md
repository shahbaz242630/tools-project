...
**Phase 4 — Availability and booking request. OPEN, 18 August 2026.** Eight of ten slices (4.5 was split into 4.5a and 4.5b): **4.1** §7's state machine as pure logic (23 states, 47 transitions, every one of the 482 forbidden pairs refused); **4.2** the `bookings` table and §8.5.1's real `EXCLUDE` constraint, with the simultaneous-acceptance test; **4.3a** the availability data and query; **4.3b** the owner's calendar page, which an owner can now open and use; **4.4a** §8.5.3's maximum rental duration as versioned configuration; **4.4b** the quote engine and the `Quote` record, which also enforces §3.4.2's minimum booking total for the first time; **4.5a** request submission — the terms a booking keeps, §6.2's immutable event history, and §8.6's request deadline as versioned configuration. **4.5b** the renter's request panel, which is the first slice in this phase with a page and the first walked in a browser. **Two thirds of the exit gate is met** — §7.1's auto-decline is 4.6. **Something is bookable and nothing is accepted.** **Two findings bind the rest of the phase**: `ACCEPTED` has no one-step release of the calendar, so acceptance must move straight through it in one transaction; and an availability block does not stop a booking at the database level, so the request and acceptance paths must consult `reasonUnavailable` themselves. **And one worth knowing anywhere**: two inserts racing on an exclusion constraint make Postgres report `40P01 deadlock detected` rather than a constraint violation about one race in three — retry, do not translate.
...
**Phases 5–12 have not been started.** No payments, messaging, handover protocol, reviews, disputes or reporting. What exists is a platform that knows who somebody is, protects what it knows about them, can be administered safely, lets an owner describe and publish an item for rent that the platform can independently hide, **shows that listing to a stranger with no account**, **lets a stranger type a postcode, narrow by category and page through what is near them**, and — from 4.3b — **lets an owner say which dates their item is not available**, from 4.4b **quotes a firm, expiring price for a period a renter chooses**, from 4.5a **turns that quote into a booking request** that carries its own terms and its own immutable history, and from 4.5b **lets a renter do all of that on the listing page itself**. It is still not a marketplace: **nothing is photographed and nobody can accept a request** — 4.6 is that slice, and it is the last third of Phase 4's exit gate. **A renter also has nowhere to see a request once made**: 4.5b's confirmation is the only place it is ever shown and a reload loses it, which is 4.8's, and the panel says so rather than pretending. **Four of thirteen phases are closed and counting them overstates progress** — 0 and 1 are scaffolding, 2 and 3 are one person describing an item and another finding it, and the booking state machine, the ledger and the condition-evidence protocol are each larger than everything built so far.
